...
functions.http('main', (req, res) => {
  ...
  cors(req, res, async () => {
    ...
    const client_id = 'YOUR_NAVER_CLIENT_ID'; // If you want to be extra secure, put this in a Google Cloud Secret
    const client_secret = 'YOUR_NAVER_CLIENT_SECRET'; // If you want to be extra secure, put this in a Google Cloud Secret
    //DO NOT ENCODE. if you add encod on the redirect_url -> there will be an error.
    const redirect_uri = 'YOUR_CALLBACK_SCHEME://login-callback'; // This is the URL that Naver will redirect to after the user logs in
    ...
    try {
      ...
      const accessToken = await fetchNaverToken(
        ...
        client_secret,
        redirect_uri
      ); // A token that lets us ask Naver about the user's details
      ...
      res.redirect(
        `${redirect_uri}?firebaseToken=${firebaseToken}&name=${encodeURIComponent(
          naverProfile.name
        )}`
      ); //}&profileImage=${encodeURIComponent(naverProfile.profile_image) //If you want to add a profile image, then please add this to the line 37.
    } catch (error) {
...
async function fetchNaverToken(
  ...
  if (!response.ok) {
    throw new Error(
      `fetchNaverToken Error: ${response.status} ${response.statusText}`
    );
  }

  if (data.error) {
    throw new Error(
      `fetchNaverToken Error: ${data.error} ${data.error_description}`
    );
  }
...
async function fetchNaverUserProfile(accessToken) {
  ...
  console.log(
    `Got the following data from Naver: ${JSON.stringify(data.response)}`
  );
  ...
  try {
    ...
    if (error.code === 'auth/user-not-found') {
      await admin.auth().createUser({
        uid: uid,
        //i purposely commented the below, as my project does not require the below
